...
class Encoder(nn.Module):
    ...
    def __init__(self, args=None):
        ...
        planes = cfg.sc_ch_dict[args.type]["planes"]
        ...
        for i in range(0, cfg.sc_ch_dict[args.type]["p"]):
        ...
        for i in range(0, cfg.sc_ch_dict[args.type]["q"]):
...
if __name__ == '__main__':
    ...
    model = TwinMixing(args).cuda()
    cudnn.benchmark = True
    model.eval()
    model = model.half()
    example = torch.randn(int(args.batch), 3, 384, 640).cuda().half()
